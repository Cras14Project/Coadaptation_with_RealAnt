...
class SoftActorCritic(RL_algorithm):
    ...
    def _create_networks(env, config):
        ...
        Returns:
            A dictonary which contains the networks.
        """
        obs_dim = int(np.prod(env.observation_space.shape))    # int(np.prod(env.get_o_space()))    #this should be mofified to match robot env DONE
        action_dim = int(np.prod(env.action_space.shape))  #int(np.prod(env.get_a_space()))     #this should be modified to match robot env DONE
