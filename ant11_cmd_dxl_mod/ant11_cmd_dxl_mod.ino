...
#define ADDR_AX_MOVING_SPEED            32 // added this
...
void read_servo_positions()
{
  ...
  // additonal failures to read data
  uint8_t speed_failures = 0;
  uint8_t load_failures = 0;
  ...
  bool result = false;
    ...
    for (int i = 1; i <= 8; i++) {
      ...
      dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, i, ADDR_AX_PRESENT_SPEED, (uint16_t*)&get_data, &dxl_error);
      if (dxl_comm_result != COMM_SUCCESS)
      {
        speed_failures |= 1<<(i-1);
      }
      Serial.print(get_data);
      Serial.print("\t");
    }
    ...
    for (int i = 1; i <= 8; i++) {
      ...
      dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, i, ADDR_AX_PRESENT_LOAD, (uint16_t*)&get_data, &dxl_error);
      if (dxl_comm_result != COMM_SUCCESS)
      {
        load_failures |= 1<<(i-1);
      }
    ...
    //removed REFERENCE/GOAL POS, TEMPERATURE, FEET PINS  from read_servo_positions
    ...
    if (speed_failures > 0)
    {
        Serial.print("- Failed to get present speed 0x");
        Serial.println(speed_failures, HEX);  
    }  
    if (load_failures > 0)
    {
        Serial.print("- Failed to get present load 0x");
        Serial.println(load_failures, HEX);  
    }   
}
...
void setup() {
    ...
    for (int i = 1; i <= 8; i++) {
        ...
        {
          packetHandler->getTxRxResult(dxl_comm_result);
          Serial.println("- Failed set max speed (1)");
          
        }
        else if (dxl_error != 0)
        {
          packetHandler->getRxPacketError(dxl_error);
          Serial.println("- Failed set max speed (2)");
        }
        else
        {
          Serial.println("- Succeeded to set max speed");
        }
    }
